            ...
            foreach($logs as $log) {
                ...
                switch ($log['action']) {
                    ...
                    case 'rate-comment':
                        ...
                        break;
                    case 'follow-request':
                        $notice_array = [u_all_info('*', $requestFromUserId)['username'].' sent a request', '<input type="button" class="access_button button_blue" id="" value="Confirm" onclick="follow_confirm('. $_SESSION['rt_UserId'].' ,'. $log['fromUserId'].')"> <input type="button" class="access_button" id="" value="Remove" onclick="remove_request('. $_SESSION['rt_UserId'].' ,'. $log['fromUserId'].')">'];
                        break;
                    case 'follow':
                        ...
                        break;
                    case 'comment':
                        $notice_array = [u_all_info('*', $requestFromUserId)['username'].' has commented on your post: "'.comment_in_post('*', $requestDataComment)[0]['comment'].'"', '<div class="frame_activity_preview"><img src="images/' .brind_post_preview(comment_in_post('*', $requestDataComment)[0]['postId'])['preview'].'" class="activity_preview"></div>'];
                        break;
                    case 'answer-comment':
                        $notice_array = ["3", 0];
                        break;
